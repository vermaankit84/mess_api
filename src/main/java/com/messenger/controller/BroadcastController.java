...
public class BroadcastController {
    ...
    ResponseEntity<List<BroadcastRequest>> getBroadcastRequestBasedOnSender(@PathVariable final String senderName) throws Exception {
        ...
        return new ResponseEntity<>(broadcastService.getBroadcastRequestBasedOnSender(sender), HttpStatus.CREATED);
    }
    ...
    @RequestMapping(value = "/updateBroadCastMessage", produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.PUT)
    public
    @ResponseBody
    ResponseEntity<BroadcastTemp> updateBroadCastMessage(@RequestBody final BroadcastTemp broadcastTemp) throws Exception {
        return new ResponseEntity<>(brdTempService.updateBroadCastMessage(broadcastTemp), HttpStatus.ACCEPTED);
    }

    @RequestMapping(value = "/updateBroadCastDetails", produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.PUT)
    public
    @ResponseBody
    ResponseEntity<Broadcast> updateBroadCastDetails(@RequestBody final BroadcastRequest broadcastRequest) throws Exception {
        return new ResponseEntity<>(broadcastService.updateBroadCastDetails(broadcastRequest), HttpStatus.ACCEPTED);
    }
}
