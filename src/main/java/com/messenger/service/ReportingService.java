...
public class ReportingService {
        ...
        if (misLogList != null) {
            misLogList.forEach(new Consumer<RecLog>() {
                ...
                public void accept(final RecLog recLog) {
                    ...
                    if (subLog != null) {
                        reportingData = new ReportingData(recLog.getMessageId(), recLog.getAppResponseId(), recLog.getDivision().getDivisionName(),
                        ...
                        reportingData = new ReportingData(recLog.getMessageId(), recLog.getAppResponseId(), recLog.getDivision().getDivisionName(),
                                recLog.getDelivaryFlag(), recLog.getMsgType(), recLog.getDestination(), recLog.getPriority(), recLog.getMessageText(),
                                recLog.getSmslength(), recLog.getDndFlag(), recLog.getReqDumpdate(), recLog.getVendorOrigin().name(), StringUtils.EMPTY, MessConstants.STR_DEFAULT_BUFFER_STATUS,
                                MessConstants.STR_DEFAULT_BUFFER_STATUS_DESC, StringUtils.EMPTY, null, null, StringUtils.EMPTY);
                    }
    ...
    @Transactional(propagation = Propagation.REQUIRES_NEW, readOnly = true, timeout = 30, rollbackFor = Exception.class)
    public List<ReportingData> getReportingData(int start, int end, final String mobile) {
        final Page<RecLog> misLogList = misLogRepository.getPageBasedMobile(new PageRequest(start, end), mobile);
        final List<ReportingData> reportingDataList = new ArrayList<>();
        if (misLogList != null) {
            misLogList.forEach(new Consumer<RecLog>() {
                @Override
